...
func TestEnvExpandNoMarkers(t *testing.T) {
	e := NewEnv(nil)
	// String with no $ or #{ should be returned as-is (fast path)
...
func TestEnvExitCodeIsolation(t *testing.T) {
	...
	// Child hasn't run anything yet — walks to parent
	if child.exitCode() != 1 {
		t.Errorf("expected 1 from parent, got %d", child.exitCode())
	}

	// Child runs true — sets its own exit
	child.setExit(0)
	if child.exitCode() != 0 {
...
func TestSplitOnCharsIFS(t *testing.T) {
	// Non-whitespace delimiter preserves empty fields
	got := splitOnChars("a::b", ":")
	if len(got) != 3 || got[0] != "a" || got[1] != "" || got[2] != "b" {
		t.Errorf("splitOnChars(\"a::b\", \":\") = %v, want [a  b]", got)
	}
	// Whitespace collapses
	got = splitOnChars("a  b  c", " ")
	if len(got) != 3 {
		t.Errorf("whitespace split: got %v, want 3 fields", got)
	}
	// Mixed IFS
	got = splitOnChars(" a : : b ", ": ")
	if len(got) != 4 || got[0] != "a" || got[1] != "" || got[2] != "" || got[3] != "b" {
		t.Errorf("mixed IFS: got %v", got)
	}
}
...
func TestEnvExitCodeWalksParent(t *testing.T) {
	...
	// Child hasn't run any commands yet — should walk to parent
	got := child.Expand("$?")
	if !strings.Contains(got, "5") {
		t.Errorf("expected exit code from parent, got %q", got)
	}

	// After child sets its own exit code, it should use that
	child.setExit(0)
	got = child.Expand("$?")
	if got != "0" {
