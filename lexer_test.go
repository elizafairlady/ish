...
func tokVals(tokens []Token) []string {
	var out []string
	for _, t := range tokens {
		if t.Type == TEOF {
			break
...
func assertTokens(t *testing.T, input string, wantTypes []TokenType, wantVals []string) {
	t.Helper()
	tokens := Lex(input)
...
func TestLexPosixAssign(t *testing.T) {
	...
	t.Run("= in argument position is part of word", func(t *testing.T) {
		...
		if tokens[1].Type != TWord || tokens[1].Val != "foo=bar" {
...
func TestLexNewlines(t *testing.T) {
	assertTokens(t, "a\nb",
		[]TokenType{TWord, TNewline, TWord},
		[]string{"a", "\n", "b"})
}
...
func TestLexHeredocBackslashNewline(t *testing.T) {
	...
	if tokens[1].Type != THeredoc {
		t.Fatalf("expected THeredoc, got %d", tokens[1].Type)
	}
	if tokens[2].Type != TString {
