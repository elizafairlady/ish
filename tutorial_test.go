...
// TestTutorialExamples validates every code example from the tutorial that has
// expected output. Each test is named by section and description.
func TestTutorialExamples(t *testing.T) {
	tests := []struct {
		...
		{"s12 map fn", "f = fn do x -> x * 2 end\nr = map [1, 2, 3], f\necho $r", "[2, 4, 6]\n"},
		{"s12 filter", "r = filter [1, 2, 3, 4, 5], fn do x -> x >= 4 end\necho $r", "[4, 5]\n"},
		{"s12 reduce", "r = reduce [1, 2, 3, 4], 0, fn do acc, x -> acc + x end\necho $r", "10\n"},
		{"s12 range filter length", "r = range 1, 11 |> filter fn do x -> x >= 6 end |> length\necho $r", "5\n"},
...
// TestTutorialPingPong tests the spawn/send/receive example from section 10.
...
// TestTutorialSelectiveReceive tests the selective receive example from section 10.
...
// TestTutorialReceiveTimeout tests the receive-with-timeout example from section 10.
...
// TestTutorialAwait tests the await example from section 10.
...
// TestTutorialMonitor tests the monitor example from section 11.
...
// TestTutorialWhile tests the while loop from section 8.
func TestTutorialWhile(t *testing.T) {
	...
	want := "3\n2\n1\n"
	if got != want {
		t.Errorf("while loop: got %q, want %q", got, want)
	}
}

// TestTutorialFromJSON tests JSON round-trip from section 12.
...
// TestTutorialToJSON tests to_json from section 12.
